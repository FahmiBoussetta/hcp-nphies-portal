...
export interface ICareTeamProps extends StateProps, DispatchProps, RouteComponentProps<{ url: string }> {}

export const CareTeam = (props: ICareTeamProps) => {
  useEffect(() => {
    props.getEntities();
  }, []);
  ...
  const { careTeamList, match, loading } = props;
  return (
    <div>
...
const mapStateToProps = ({ careTeam }: IRootState) => ({
  careTeamList: careTeam.entities,
