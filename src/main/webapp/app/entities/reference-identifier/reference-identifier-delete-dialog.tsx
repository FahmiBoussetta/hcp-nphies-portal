...
export interface IReferenceIdentifierDeleteDialogProps extends StateProps, DispatchProps, RouteComponentProps<{ id: string }> {}

export const ReferenceIdentifierDeleteDialog = (props: IReferenceIdentifierDeleteDialogProps) => {
  useEffect(() => {
  ...
  const confirmDelete = () => {
    props.deleteEntity(props.referenceIdentifierEntity.id);
  };
...
const mapStateToProps = ({ referenceIdentifier }: IRootState) => ({
  ...
  updateSuccess: referenceIdentifier.updateSuccess,
});
...
export default connect(mapStateToProps, mapDispatchToProps)(ReferenceIdentifierDeleteDialog);
