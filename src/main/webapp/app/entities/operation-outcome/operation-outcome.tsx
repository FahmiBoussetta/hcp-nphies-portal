  ...
  return (
    <div>
      <h2 id="operation-outcome-heading" data-cy="OperationOutcomeHeading">
        <Translate contentKey="hcpNphiesPortalApp.operationOutcome.home.title">Operation Outcomes</Translate>
        <div className="d-flex justify-content-end">
          <Button className="mr-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="hcpNphiesPortalApp.operationOutcome.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to={`${match.url}/new`} className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="hcpNphiesPortalApp.operationOutcome.home.createLabel">Create new Operation Outcome</Translate>
          </Link>
        </div>
      </h2>
      <div className="table-responsive">
        {operationOutcomeList && operationOutcomeList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                <th>
                  <Translate contentKey="hcpNphiesPortalApp.operationOutcome.id">ID</Translate>
                </th>
                <th>
            ...
            <tbody>
              {operationOutcomeList.map((operationOutcome, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  <td>
                    <Button tag={Link} to={`${match.url}/${operationOutcome.id}`} color="link" size="sm">
                      {operationOutcome.id}
                    </Button>
                  </td>
                  ...
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${operationOutcome.id}/edit`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${operationOutcome.id}/delete`}
                        color="danger"
                        size="sm"
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="hcpNphiesPortalApp.operationOutcome.home.notFound">No Operation Outcomes found</Translate>
            </div>
          )
        )}
      </div>
